  ...
  <div class="px-2 py-2">
    ...
    <div v-else class="table-responsive">
      <table class="table table-hover table-bordered align-middle">
        ...
        <tbody>
          <tr v-for="(item, index) in highlightsData" :key="item.id">
...
function editItem(item) {
  router.push({ path: '/admin/highlights/update', query: { id: item.id } })
}
const deleteItem = (id) => {
  Swal.fire({
    title: 'Are you sure?',
    ...
    if (result.isConfirmed) {
      axios.delete(`${apiBaseUrl}/api/highlights/delete?id=${id}`)
      .then((response) => {
...
const fetchHighlights = async () => {
  ...
  try {
    ...
    const res = await fetch(`${apiBaseUrl}/api/highlights?${params}`)
    const result = await res.json()

    highlightsData.value = result.data
